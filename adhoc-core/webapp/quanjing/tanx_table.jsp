...
function requestPV_AVG_pid()
{
		var strproducttype=jQuery("#producttypecmp").val();
	  ...
	  var requestparams={};
		...
		requestparams.project="rpt_quanjing_tanxpv";
			...
			requestparams.fl="pid,sum(records),dist(thedate)";
			...
			$.post("/result.jsp",requestparams,
					function (data, textStatus){	
						...
						for(var i=0;i<listtmp.length;i++)
						{
							...
							if(!returnresult[item['pid']])
							{
								returnresult[item['pid']]={'v':records/days};
							}
						}
						searchDatatblClick("pv_avg",returnresult,'pv',1);
						showresultable("pv_avg",returnresult,'pv',1);
...
function requestCLICK_AVG_pid(pidlist)
{
		var chooseday=jQuery("#thedateCmp").val();
		var datts=dayToTimestampDay(chooseday);
		var daystart=parseDay(new Date(datts-8*24*60*60*1000));
		  ...
		  requestparams.q=JSON.stringify(newq);
			...
			$.post("/result.jsp",requestparams,
					function (data, textStatus){	
						...
						showresultable("click_avg",returnresult,'click',3);
...
function requestPV_today_pid()
{
	  ...
	  var requestparams={};
		...
		requestparams.project=strtablename;
			...
			requestparams.fl="pid,producttype,sum(records)";
		  requestparams.q=JSON.stringify(newq);
			...
			$.post("/result.jsp",requestparams,
					function (data, textStatus){	
						...
						for(var i=0;i<listtmp.length;i++)
						{
							...
							if(!returnresult[item['pid']])
							{
								returnresult[item['pid']]={'producttype':item["producttype"],'v':records};
							}
						}
						
						searchDatatblClick("pv_today",returnresult,'pv',2);
						showresultable("pv_today",returnresult,'pv',2);
				}, "json");
}



function requestCLICK_today_pid(pidlist)
{
		var strproducttype=jQuery("#producttypecmp").val();
		var chooseday=jQuery("#thedateCmp").val();
		var datts=dayToTimestampDay(chooseday);
	  ...
	  var requestparams={};
		...
		requestparams.project=strtablename;
			...
			requestparams.sort="sum(records)";
			requestparams.groupby="pid";
			requestparams.fl="pid,sum(records)";
		  requestparams.q=JSON.stringify(newq);
			...
			$.post("/result.jsp",requestparams,
					function (data, textStatus){	
						...
						for(var i=0;i<listtmp.length;i++)
						{
							...
							if(!returnresult[item['pid']])
							{
								returnresult[item['pid']]={'v':records};
							}
						}
						showresultable("click_today",returnresult,'pv',4);
				}, "json");
}
...
var today=new Date();
jQuery("#thedateCmp").val(parseDay(new Date(today.getTime())));
...
{
	$("#tablelist_id").empty();
$("#tablelist_id").html('<table id="list4"  dir="ltr" style="width: 670px;"></table><div id="pager2"></div>');
...
function searchDatatblClick(tp,data,YPOS,color)
{
	g_result_table_pid[tp]=data;
		var count=0;
	for(var p in g_result_table_pid)
	{
		count++;
	}
	if(count<2)
	...
	var pidlistmap={};
	var pvtoday=g_result_table_pid['pv_today'];
	var pvavg=g_result_table_pid['pv_avg'];
...
for(var p in pidlistmap)
	...
	requestCLICK_AVG_pid(pidlist);
	requestCLICK_today_pid(pidlist);
}


function showresultable(tp,data,YPOS,color)
{
	
	g_result_table[tp]=data;
	
	var count=0;
	for(var p in g_result_table)
	{
		count++;
	}
	...
	var resultlist=[];
	var pvtoday=g_result_table['pv_today'];
	var pvavg=g_result_table['pv_avg'];
	var clicktoday=g_result_table['click_today'];
